...
const columns = [
  ...
  },
  {
    ...
    dataIndex: 'outstandingShares',
    key: 'outstandingShares',
  },
  {
    title: 'Market Cap',
    key: 'marketCap',
    render: (text, record) => calculateMarketCap(record),
  },
];

const calculateMarketCap = (stock) => {
...
// [...stockData] creates a copy of the stockData array
// This useMemo only runs when the sortOrder changes (not on every re render)
const sortedStocks = useMemo(() => [...stockData].sort((a, b) => {
const sortingByMarketCap = sortOrder.property === 'marketCap';
const aValue = sortingByMarketCap ? calculateMarketCap(a) : a[sortOrder.property];
const bValue = sortingByMarketCap ? calculateMarketCap(b) : b[sortOrder.property];
return (
sortOrder.order === 'asc' ? aValue - bValue : bValue - aValue
);
}), [sortOrder]);
...
sortedStocks.filter((stock) => stock.sector === 'Technology') :
sortedStocks;

// Calculate the total outstanding shares
const totalOutstandingShares = filteredStocks.reduce((totalShares, stock) => {
...
<div className={styles.main}>
  ...
  <div className={styles.buttonContainer}>
    <div className={styles.sortButtonContainer}>
      ...
      <Button onClick={() => sortByProperty('marketCap')}>Sort by Current Value</Button>
    </div>
